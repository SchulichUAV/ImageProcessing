...
def undestortFactor(i,j, imageSize):
    distance = distanceFromCenter(i,j)
    value = 1+3.74510846e-20*distance**2+.04472008e-14*distance**4+1.80526733e-20*distance**6#math.cos(parameter)*parameter #radius*angle #2*math.pi/360
    return value*0.8


imgInName = 'IMAG0016.JPG'
...
for i in range(-xCent, xCent):
    for j in range(-yCent, yCent):
        imageSize = maxSize
        s = undestortFactor(i,j, imageSize)
